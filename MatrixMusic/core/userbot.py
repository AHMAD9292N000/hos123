...
class Userbot(Client):
    ...
    async def start(self):
        LOGGER("ميوزك يزن").info(f"جارِ تشغيل الحساب المساعد . . .")
        if config.STRING1:
            await self.one.start()
            try:
                await self.one.join_chat("CZCRR0")
                await self.one.join_chat("CZCRR0")
            except:
                pass
            assistants.append(1)
        ...
        if config.STRING2:
            ...
            LOGGER("ميوزك حسام").info(f"Assistant Two Started as {self.two.name}")
        ...
        if config.STRING3:
            ...
            LOGGER("ميوزك حسام").info(f"Assistant Three Started as {self.three.name}")
        ...
        if config.STRING4:
            ...
            LOGGER("ميوزك حسام").info(f"Assistant Four Started as {self.four.name}")
    ...
    async def stop(self):
        LOGGER("ميوزك حسام").info(f"Stopping Assistants...")
        try:
            if config.STRING1:
                await self.one.stop()
            if config.STRING2:
